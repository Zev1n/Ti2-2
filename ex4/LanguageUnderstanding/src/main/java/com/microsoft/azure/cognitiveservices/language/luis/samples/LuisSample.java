...
/**
 * Sample code for spell checking using Language Understanding (LUIS), an Azure
 * Cognitive Service.
 * - Create an application with two entities.
 * - Create a "Flight" composite entity including "Class" and "Destination"
 * - Create a new "FindFlights" intent with two utterances and build two
 * EntityLabel Object
 * - Train and publish the application
 * - Execute a LUIS prediction for a "find second class flight to new york"
 * utterance and print the results
 * - Execute a LUIS prediction for a "find flights to London in first class"
 * utterance and print the results
 */
public class LuisSample {
    ...
    public static boolean runLuisAuthoringSample(LUISAuthoringClient authoringClient) {
        try {
            ...
            System.out.println(
                    "\t\"Class\" - hierarchical entity will accept \"First\", \"Business\" and \"Economy\" values");
            ...
            // =============================================================
            // This will create the "Flight" composite entity including "Class" and
            // "Destination"
            ...
            // =============================================================
            // This will create a new "FindFlights" intent including the following
            // utterances
            ...
            ExampleLabelObject exampleLabelObject2 = new ExampleLabelObject()
                    ...
                    .withIntentName(intentName)
                    .withEntityLabels(Arrays.asList(
                            getEntityLabelObject(utteranceFindFirstToLondon, "Flight",
                                    "London in first class in first class"),
            ...
            while (!isTrained) {
                ...
                for (ModelTrainingInfo modelTrainingInfo : status) {
                    if (!modelTrainingInfo.details().status().equals("UpToDate")
                            && !modelTrainingInfo.details().status().equals("Success")) {
                        isTrained = false;
                        break;
                    }
                }
            ...
            ProductionOrStagingEndpointInfo publishResult = authoringClient.apps().publish(appId,
    ...
    public static boolean runLuisRuntimeSample(LuisRuntimeAPI runtimeClient) {
        try {
            ...
            // =============================================================
            // This will execute a LUIS prediction for a "find second class flight to new
            // york" utterance
            ...
            if (predictionResult != null && predictionResult.topScoringIntent() != null) {
                System.out.format("Detected intent \"%s\" with the score %f%%\n",
                        predictionResult.topScoringIntent().intent(),
                        predictionResult.topScoringIntent().score() * 100);
            ...
            // =============================================================
            // This will execute a LUIS prediction for a "find flights to London in first
            // class" utterance
    ...
    public static void main(String[] args) {
        try {
            ...
            luisAuthoringKey = System.getenv("4acab49e-58a2-4b88-8c78-16ed37a0ee9b");

            LUISAuthoringClient authoringClient = LUISAuthoringManager
                    .authenticate(com.microsoft.azure.cognitiveservices.language.luis.authoring.EndpointAPI.US_WEST,
                            luisAuthoringKey);
            LuisRuntimeAPI runtimeClient = LuisRuntimeManager
                    .authenticate(com.microsoft.azure.cognitiveservices.language.luis.runtime.EndpointAPI.US_WEST,
                            luisAuthoringKey);
